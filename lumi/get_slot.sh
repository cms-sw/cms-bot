...
#--bind $SINGULARITY_SCRATCH:/workspace:image-src=/ "$SINGULARITY_CACHEDIR/cmssw_${OS}.sif" $(dirname $0)/jenkins_java.sh ${JENKINS_JAR}

/project/project_462000245/local/bin/cms-${OS}-exec --bind $SINGULARITY_SCRATCH:/workspace:image-src=/ $(dirname $0)/jenkins_java.sh ${JENKINS_JAR}
