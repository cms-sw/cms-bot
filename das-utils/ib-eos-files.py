...
def get_lfns_from_kibana(days=7):
    ...
    cmd = "PYTHONPATH=%s/.. %s/ib-datasets.py --days %s > %s; cat %s" % (
        CMS_BOT_DIR,
        CMS_BOT_DIR,
        days,
        kibana_file,
    ...
    if exists(kibana_file):
        cmd = "cat %s" % kibana_file
...
def get_lfns_from_das(lfn_per_query=1):
    ...
    err, out = run_cmd(
        "test -d cms-sw.github.io || git clone --depth 1 https://github.com/cms-sw/cms-sw.github.io.git"
    )
    err, qfiles = run_cmd("ls cms-sw.github.io/das_queries/*/*.query")
    used_lfns = {}
    for qfile in qfiles.split("\n"):
        lfn_file = qfile.strip()[:-6]
        if not exists(lfn_file):
            continue
        lfn_count = 0
...
def eos_rename(name, new_name):
    ...
    err, out = run_cmd(
        "%s file rename %s %s" % (eos_cmd, name, new_name), exit_on_error=False, debug=False
    )
    if err:
        print(out)
        return False
...
def copy_lfns_to_eos(eos_lfns):
    ...
    for lfn in eos_lfns:
        ...
        if eos_exists(eos_file) or (
            eos_exists(eos_file + ".unused") and eos_rename(eos_file + ".unused", eos_file)
        ):
            already_done += 1
    ...
    for log in all_logs:
        ...
        if out == "0":
            total_failed += 1
            print(
                "FAIL (%s/%s): %s" % (already_done + total_copied + total_failed, total_lfns, lfn)
            )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="dryRun",
        action="store_true",
        help="Do not actually download the files",
        default=False,
    )
    parser.add_option(
        "-f",
        "--file-per-das",
        dest="files_per_das",
        help="Number of files per das query need to be copy to EOS",
        type=int,
        default=1,
    )
    parser.add_option(
        "-j", "--jobs", dest="jobs", help="Parallel jobs to run", type=int, default=4
    )
    parser.add_option(
        "-d",
        "--days",
        dest="days",
        help="Files access in last n days via kibana",
        type=int,
        default=7,
    )
