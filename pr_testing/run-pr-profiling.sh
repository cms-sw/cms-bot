...
EOF
  ...
  for f in $(find $PROFILING_WORKFLOW -type f -name 'sorted_RES_CPU_step*.json' | sort -V ) ; do
    ...
    if [ -f $CMSSW_VERSION-$BASENAME ] ; then
      ...
      echo "<td><a target=\"_blank\" href=\"${PROFILING_WORKFLOW}/diff-$BASENAME\">diff-${BASENAME/.json/.html}</a></td>" >> $WORKSPACE/upload/profiling/index-$PROFILING_WORKFLOW.html || true
    else
