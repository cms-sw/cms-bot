...
TESTS_RESULTS_MSG = r"^\s*([-|+]1|I had the issue.*)$"
...
CLOSE_REQUEST = re.compile(r"^((@|)cmsbuild\s*[,]* |)(please\s*[,]* |)close$", re.I)
REOPEN_REQUEST = re.compile(r"^((@|)cmsbuild\s*[,]* |)(please\s*[,]* |)(re|)open$", re.I)
...
TEST_REGEXP = format(
    r"^((@|)cmsbuild\s*[,]* |)(please\s*[,]* |)test( workflow(s|) (%(workflow)s(\s*,\s*%(workflow)s|)*)|)( with (%(cms_pr)s(\s*,\s*%(cms_pr)s)*)|)( for %(release_queue)s|)( using full cmssw| using (cms-|)addpkg (%(pkg)s(,%(pkg)s)*)|)$",
    workflow=WF_PATTERN,
...
REGEX_TEST_ABORT = re.compile(r"^((@|)cmsbuild\s*[,]* |)(please\s*[,]* |)abort( test|)$", re.I)
REGEX_TEST_IGNORE = re.compile(
    r"^(?:(?:@|)cmsbuild\s*[,]* |)(?:please\s*[,]* |)ignore tests-rejected (?:with|)([a-z -]+)$",
    re.I,
)
...
def get_assign_categories(line, extra_labels):
    m = re.match(
        r"^(New categories assigned:\s*|(unassign|assign) (from |package |))([a-zA-Z0-9/,\s-]+)$",
        line,
        re.I,
    )
    if m:
...
def check_extra_matrix_args(first_line, repo, params, mkey, param, *args):
    kitem = mkey.split("_")
    logging.debug(
        f"check_extra_matrix_args called with: {first_line=}, {repo=}, {params=}, {mkey=}, {param=}"
    )
    if kitem[-1] in ["input"] + EXTRA_RELVALS_TESTS:
        param = param + "_" + kitem[-1].upper().replace("-", "_")
    logging.debug(f"check_extra_matrix_args: {first_line=}, {param=}")
...
def check_matrix_extras(first_line, repo, params, mkey, param, *args):
    kitem = mkey.split("_")
    logging.debug(
        f"check_matrix_extras called with: {first_line=}, {repo=}, {params=}, {mkey=}, {param=}"
    )
...
def multiline_check_function(first_line, comment_lines, repository):
    ...
    logging.debug(f"multiline_check_function: {extra_params=}")
...
def process_pr(repo_config, gh, repo, issue, dryRun, cmsbuild_user=None, force=False):
    ...
    pkg_categories = set([])
    REGEX_TYPE_CMDS = (
        r"^(type|(build-|)state) (([-+]|)[a-z][a-z0-9_-]+)(\s*,\s*([-+]|)[a-z][a-z0-9_-]+)*$"
    )
    REGEX_EX_CMDS = r"^urgent$|^backport (of |)(#|http(s|):/+github\.com/+%s/+pull/+)\d+$" % (
        repo.full_name
    )
    known_ignore_tests = "%s" % MULTILINE_COMMENTS_MAP["ignore_test(s|)"][0]
    REGEX_EX_IGNORE_CHKS = r"^ignore ((%s)(\s*,\s*(%s))*|none)$" % (
        known_ignore_tests,
        known_ignore_tests,
    )
    REGEX_EX_ENABLE_TESTS = r"^enable (%s)$" % MULTILINE_COMMENTS_MAP[ENABLE_TEST_PTRN][0]
    ...
    for comment in all_comments:
        ...
        if commenter == cmsbuild_user:
            ...
            if m := re.match(HOLD_MSG, first_line):
        ...
        if re.match("^hold$", first_line, re.I):
            ...
            if valid_commenter:
                ...
                comment_emoji = "+1"
        elif m := re.match(r"^allow @(\S+) test rights$", first_line, re.I):
            comment_emoji = "-1"
            if commenter_categories or (commenter in releaseManagers):
                ...
                if not tester in TRIGGER_PR_TESTS:
                    ...
                    logging.info(f"Granted user {tester} permission to run tests")
                comment_emoji = "+1"
        ...
        elif re.match(r"^" + REGEX_IGNORE_COMMIT_COUNT + r"$", first_line):
            comment_emoji = "-1"
            if commenter in CMSSW_ISSUES_TRACKERS:
                ok_too_many_commits = pr.commits < TOO_MANY_COMMITS_FAIL_THRESHOLD
                comment_emoji = "+1"
        elif re.match(r"^" + REGEX_IGNORE_FILE_COUNT + r"$", first_line):
            comment_emoji = "-1"
            if commenter in CMSSW_ISSUES_TRACKERS:
        ...
        if cmssw_repo:
            if m := CODE_CHECKS_REGEXP.match(first_line):
        ...
        if issue.pull_request or push_test_issue:
            ...
            if re.match(r"^(merge)$", first_line, re.I):
                emoji = "-1"
    ...
    old_labels = set([x.name.encode("ascii", "ignore").decode() for x in issue.labels])
    logging.info(f"Stats: {backport_pr_num=}, {extra_labels=}, {state_labels=}")
    ...
    if "tests" in signatures:
        if test_comment is not None:
            ...
            if bot_status:
                ...
                logging.debug(f"  {bot_status=}")
                logging.debug(f"  {bot_status.description=}")
                logging.debug(f"  {bot_status.target_url=}")
                logging.debug(f"  {test_comment.html_url=}")
            ...
            if bot_status:
                logging.debug(
                    f"{bot_status.target_url=}, {turl=}, {signatures['tests']=}, {bot_status.description=}"
                )
            if (
                bot_status
            ...
            if signatures["tests"] == "started" and new_bot_tests:
                ...
                for status in commit_statuses:
                    ...
                    logging.info(
                        f"Final Status: {status.context=} {cdata[-1]=} {lab_stats[cdata[-1]][-1]=} {status.description=}"
                    )
    ...
    for pre_check in pre_checks + extra_pre_checks:
        ...
        logging.debug(
            f"PRE CHECK: {pre_check=},signatures={signatures[pre_check]},state={pre_checks_state[pre_check]}"
        )
        ...
        if signatures[pre_check] != "pending":
            if pre_checks_state[pre_check] in ["pending", ""]:
                ...
                logging.info(f"Setting status: {pre_check=},{state=},{url=}")
                if not dryRunOrig:
                    last_commit_obj.create_status(
                        state,
                        target_url=url,
