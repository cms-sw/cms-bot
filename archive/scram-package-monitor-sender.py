...
def esReportPackages(results):
    ...
    request = Request(url)
    if es_auth:
        base64string = base64.encodebytes(es_auth.endcode()).replace(b"\n", b"").decode()
        request.add_header("Authorization", "Basic %s" % base64string)
    request.get_method = lambda: "POST"
    data = "\n".join(results) + "\n"
    try:
        result = urlopen(request, data=data.encode())
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-f", "--force", dest="force", action="store_true", help="Force pushing", default=False
    )
    parser.add_option(
        "-n",
        "--dry-run",
        dest="dryrun",
        action="store_true",
        help="Do not push files to server",
        default=False,
    )
    ...
    while True:
        ...
        timestamps = sorted(
            [int(basename(x).split("_")[1].split("-")[0]) for x in glob(join(WORK_DIR, "st*"))]
        )
        ...
        if m:
            ...
            defaults["@timestamp"] = strftime(
                "%Y-%m-%dT%H:%M:00.0", strptime(m.group(2), "%Y-%m-%d-%H%M")
            )
        ...
        for x in packages:
            ...
            header = {"index": {"_index": INDEX_NAME, "_type": "cmssw_pkg_times", "_id": h}}
            data = {"package": x}
