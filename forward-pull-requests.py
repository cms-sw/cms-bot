...
if __name__ == "__main__":
  ...
  for pr in pulls:
    ...
    newBody = pr.body + "\nAutomatically ported from " + args.source + " #%s (original by @%s).\nPlease wait for a new IB (12 to 24H) before requesting to test this PR." % (pr.number, str(pr.head.user.login))
    try:
      ...
      request = Request("https://api.github.com/repos/cms-sw/cmssw/pulls",
