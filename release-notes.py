  ...
  for pr_number in notes:
    categories = [ l.split('-')[0] for l in cache[pr_number]['pr']['labels']
                                       if (re.match("^[a-zA-Z0-9]+[-](approved|pending|hold|rejected)$", l)
                                          and not re.match('^(tests|orp)-' , l)) or l in dpg_pog_labels ]
    if len(categories) == 0:
...
def get_cmssw_notes( previous_release , this_release, cache ):
  if not exists("cmssw.git"):
    error, out = run_cmd("git clone --bare --reference /cvmfs/cms-ib.cern.ch/git/cms-sw/cmssw.git git@github.com:cms-sw/cmssw.git")
...
if __name__ == "__main__":
  ...
  try:
    ...
    if not opts.dryRun:
      ...
      print(urlopen(request, json.dumps({"body": header + cmssw_notes_str + cmsdist_header + cmsdist_notes_str }).encode()).read())
    else:
      print(header)
