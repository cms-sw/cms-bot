...
def process(repo, pr, detail=False):
  if not check_commits_files(repo, pr, detail): return False
  return True
...
if __name__ == "__main__":
  ...
  parser.add_option("-d", "--detail",     dest="detail",     action="store_true", help="Print detail output", default=False)
  opts, args = parser.parse_args()

  if len(args) != 1: parser.error("Too many/few arguments")
  if not process(opts.repository, args[0], opts.detail): sys.exit(1)
