...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cookie-jar", dest="jar", default=COOKIE_JAR, help="Where to find the cookie jar."
    )
    ...
    print(args.api[0])
    json = json.dumps(
        {"parameter": [dict(list(zip(["name", "value"], x.split("=")))) for x in args.args]}
    )
    print(json)
    cmd = format(
        API_CMD,
        ...
        api=args.api[0],
        json=json,
        token=os.getenv("HUBOT_JENKINS_TOKEN"),
    )
    print(cmd)
    err, out = run_cmd(cmd)
    if err:
