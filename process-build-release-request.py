...
WRONG_NOTES_RELEASE_MSG = 'Previous release "{previous_release}" does not appear to be a valid release name'
...
#
def create_properties_files_upload( release_name, arch, issue_number, machine_name, docker_imgs, prod ):

  docker_img = ''
  if arch in docker_imgs: docker_img = docker_imgs[arch]
...
def search_date_comment( comments, user_logins, pattern, first_line ):
  ...
  for comment in reversed( comments ):
    ...
    if first_line:
      examined_str = str(comment.body.encode("ascii", "ignore").decode().split("\n")[0].strip("\n\t\r "))
...
def search_in_comments( comments, user_logins, pattern, first_line ):
  ...
  for body in requested_comment_bodies:
    examined_str = body
    if first_line:
      examined_str = str(body.encode("ascii", "ignore").decode().split("\n")[0].strip("\n\t\r "))
...
#
def create_properties_files( issue, release_name, architectures, issue_number, queue, docker_imgs, only_toolconf=False, cmsdist_commit=None):
  ...
  for arch in architectures:
    ...
    if cmsdist_commit: out_file.write( 'CMSDIST_HASH=%s\n' % cmsdist_commit )
...
def get_release_github(repository, release_name):
  ...
  request = Request("https://api.github.com/repos/" + GH_CMSSW_ORGANIZATION + "/" + GH_CMSSW_REPO +"/releases/tags/"+release_name)
  try:
...
def create_release_github( repository, release_name, branch, commit=None):
  if get_release_github (repository, release_name): return True
  if opts.dryRun:
...
def delete_tag(org, repo, tag):
  if not exists (repo):
    cmd = "mkdir deltag-{repo}; cd deltag-{repo}; git init; git remote add {repo} git@github.com:{org}/{repo}.git".format(org=org,repo=repo)
    print('Executing: \n %s' % cmd)
...
def triggerCleanup( issue, comments, release_name ):
  ...
  if TO_CLEANUP:
    ...
    for arch in TO_CLEANUP:
      ...
      tool_conf_info_comments = search_in_comments( comments, ['cmsbuild'], pattern_tool_conf, False)
...
#
def start_release_build( issue, release_name, release_branch, architectures, docker_imgs, commit=None, cmsdist_commit=None):
  ...
  msg = RELEASE_CREATED_MSG.format( rel_name=(RELEASE_BASE_URL % release_name),
                                    base_branch=(BASE_BRANCH_URL % release_branch) )
  post_message( issue , msg )

  ready_to_build = list( set( architectures ) - set( TOOL_CONF_WAITING ) - set( TOOL_CONF_ERROR ) - set( TOOL_CONF_BUILDING ) )
  create_properties_files( issue, release_name, ready_to_build, issue.number, release_queue, docker_imgs, only_toolconf=False, cmsdist_commit=cmsdist_commit )
  if ready_to_build:
...
#
def start_tool_conf_build( issue, release_name, release_branch, architectures, docker_imgs, cmsdist_commit=None):

  create_properties_files( issue, release_name, architectures, issue.number, release_queue, docker_imgs, only_toolconf=True, cmsdist_commit=cmsdist_commit)
...
def fix_release_description(issue):
  ...
  for l in desc_str.split('\n'):
    ...
    if "PRODUCTION_ARCHITECTURE:" in l: continue
    desc_lines.append(l)
  return "\n".join(desc_lines)
...
def generate_announcement_link( announcement, release_name ):
  ...
  subject = quote(ANNOUNCEMENT_EMAIL_SUBJECT.format( rel_type=type_str,
                                is_patch=patch_str,
                                rel_name=release_name))
...
def check_if_prod_arch_ready( issue, prev_rel_name, production_architecture ):
  if ( production_architecture in INSTALL_OK ):
    ...
    announcement = generate_announcement( release_name, prev_rel_name, production_architecture, list(set(INSTALL_OK+UPLOAD_OK)) )
...
def check_archs_to_upload( release_name, issue, docker_imgs, production_architecture):
  ...
  for arch in BUILD_OK:
    ...
    first_line_info_comment = str(build_info_comments[-1].encode("ascii", "ignore").decode().split("\n")[0].strip("\n\t\r "))
    ...
    create_properties_files_upload( release_name , arch , issue.number , build_machine, docker_imgs, arch==production_architecture )
...
def check_to_build_after_tool_conf( issue, release_name, release_queue, docker_imgs):
  ...
  create_properties_files( issue, release_name, ready_to_build, issue.number, release_queue, docker_imgs )
  if ready_to_build:
    msg = QUEUING_BUILDS_MSG % ', '.join( ready_to_build )
    post_message( issue , msg )
...
def guess_prev_rel_name( release_name, issue ):
  ...
  rel_match = re.sub("_"+num_str+"$","_"+prev_num_str,release_name)+"\(_[a-zA-Z]*patch[0-9][0-9]*\|\);"
  if number == 0:
    rel_match = release_name + "_pre\([0-9][0-9]*\);"
  ret, out = run_cmd("grep 'label="+ rel_match +"' "+ CMS_BOT_DIR+"/releases.map" + " | grep -v 'label=" + release_name + ";' | tail -1 | sed 's|.*label=||;s|;.*||'")
  return out
...
if __name__ == "__main__":
  ...
  print(release_name)
  issue_body = ""
  if issue.body: issue_body = issue.body.encode("ascii", "ignore").decode().strip()
  release_queue = None
  ...
  if "ARCHITECTURE:" in issue_body:
    sel_archs = set(issue_body.split("ARCHITECTURE:",1)[1].split("\n",1)[0].strip().split(","))
  ...
  architectures = [x["SCRAM_ARCH"] for x in specs
                   if x["RELEASE_QUEUE"] == release_queue]
  ...
  docker_imgs = {}
  for x in specs:
    if (x["RELEASE_QUEUE"] == release_queue) and ("DOCKER_IMG" in x) and (x["SCRAM_ARCH"] in architectures):
  ...
  if len(architectures)>1:
    err, production_architecture = run_cmd(CMS_BOT_DIR+"/get-production-arch %s %s" % (release_name, release_queue))
  ...
  if "PRODUCTION_ARCHITECTURE:" in issue_body:
    ...
    if not req_arch in architectures:
        msg = "You requested production architecutre to be %s but this is not a valid architecture for this release cycle." % req_arch
        print(msg)
        post_message( issue, msg)
        exit(0)
    if is_patch and (production_architecture != req_arch):
        msg="You can not override production architecture for a patch release.\nProdction architecture for this release should be %s" % production_architecture
        print(msg)
        post_message( issue, msg)
        exit(0)
  ...
  if is_patch:
    try:
      ...
      if get_branch(cmssw_repo_name, possible_branch)['name'] == possible_branch:
        ...
        print('This is a patch release and the branch %s was found. It will be used as base branch.' % possible_branch)
  ...
  for rm in get_release_managers(release_branch):
    if not rm in APPROVE_BUILD_RELEASE: APPROVE_BUILD_RELEASE.append(rm)
    if not rm in REQUEST_BUILD_RELEASE: REQUEST_BUILD_RELEASE.append(rm)
  ...
  if status == NEW_ISSUE:
    ...
    if release_queue != release_branch: xqueue =release_queue+"("+release_branch+")"
    cmssw_commit_tag=""
    if release_tag_commit: cmssw_commit_tag="Release tag based on: %s\n" % release_tag_commit
    arch_msg = ", ".join([ a if a!=production_architecture else '%s(%s)' % (a,'Production') for a in architectures ])
    msg = ACK_MSG.format( approvers_list=approvers, cmssw_queue=xqueue, architecture=arch_msg, cmssw_commit_tag=cmssw_commit_tag )
  ...
  if status == PENDING_APPROVAL:
    ...
    if build_toolconf_commments:
      if is_patch:
        ...
        start_tool_conf_build( issue, release_name, release_branch, architectures, docker_imgs, cmsdist_commit=cmsdist_tag_commit)
        go_to_state( issue, status, TOOLCONF_BUILDING )
    elif approval_comments:
      start_release_build( issue, release_name, release_branch, architectures, docker_imgs, release_tag_commit, cmsdist_commit=cmsdist_tag_commit)
  ...
  if status == UPLOADING_BUILDS:
    ...
    if INSTALL_OK and ( RELEASE_NOTES_GENERATED_LBL not in labels ):
      ...
      if release_notes_comments:
        ...
        first_line = str(comment.encode("ascii", "ignore").decode().split("\n")[0].strip("\n\t\r "))
