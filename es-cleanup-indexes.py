...
if __name__ == "__main__":
  ...
  parser.add_argument("--auth-file", dest="authFile", help="file containing the authentication token")
  parser.add_argument("--dry-run", "-n", dest="dryrun", action="store_true", 
                      default=False, help="the elasticsearch entrypoint")
  ...
  cmd = format("curl -s %(proxy_string)s %(server)s/_cat/indices "
               "%(user_string)s",
  ...
  for c in CONFIG:
    ...
    for index in indices:
      ...
      if not args.dryrun:
        cmd = format("curl -s -X DELETE %(proxy_string)s %(server)s/%(index)s"
                     " %(user_string)s",
                     proxy_string=proxy_string,
                     server=args.server,
                     index=index,
