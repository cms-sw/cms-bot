...
# ================================================================================
    ...
    def generateHTML(self):
        ...
        for pk in packs:
            ...
            if self.AppSet.has_key(pk) and self.BldSet.has_key(pk):
                res[4].append(pk)
                continue
            elif self.BldSet.has_key(pk): offset = 2
            if pData['binary']['packages'].has_key(pk): res[offset+1].append(pk)
            else: res[offset].append(pk)
	...
	outFile.write("</tr> </tbody></table>\n")
        ...
        style = 'ok'
	if   rCount[0]>0: style = 'error'
	elif rCount[1]>0: style = 'warning'
	...
	for i in range(0,6):
		...
		outFile.write("<tr><td %(class)s> %(count)d </td><td> %(pack)s </td>" % {'class' :rStyle[i] , 'count' : cnt, 'pack': pack})
...
def usage():
    print "usage:", os.path.basename(sys.argv[0]), " --ignominy <ignominyDir> --release <releaseDir> --cmsdist <dir> --application <fwlite|online>"
    return
