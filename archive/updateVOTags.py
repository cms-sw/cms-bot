...
def withGridEnv(command, **kwds):
    opts = {
        "environment": "/afs/cern.ch/cms/LCG/LCG-2/UI/cms_ui_env.sh",
        "command": command % kwds,
    }
    return subprocess.getstatusoutput("source %(environment)s ; %(command)s" % opts)


def availableCes():
    error, out = withGridEnv(
        "lcg-info --vo cms --list-ce"
...
def gridReleases(ce):
    ...
    if error:
        return None
    return ["CMSSW_" + x.split("CMSSW_")[1] for x in out.split("\n") if "CMSSW_" in x]


def announceRelease(ce, release):
    error, out = withGridEnv(
        "lcg-tags --ce %(ce)s --vo cms --add --tags VO-cms-%(release)s", ce=ce, release=release
    )
...
if __name__ == "__main__":
    ...
    error, out = withGridEnv("voms-proxy-init -voms %(voms)s", voms="cms:/cms/Role=lcgadmin")
    if error:
    ...
    for ce in ces:
        ...
        errors += [
            "Release %s cannot be announced on %s" % (x, ce) for (x, err) in announced if err
        ]
        ok = ["Release %s announced." % (x, ce) for (x, err) in announced if err]
        if not errors:
            print("\n".join(ok))
            break
