...
for f in glob.glob("%s/upload/*/*/*cpu*.json" % os.getenv("WORKSPACE")):
    ...
    with open(f, "r") as file:
        ...
        payload["time_thread"] = float(total.get("time_thread", 0.0))
        payload["time_real"] = float(total.get("time_real", 0.0))
        ...
        payload["arch"] = arch
        payload["hash"] = rhash
        notification, _, _ = stomp_amq.make_notification(
            payload, "profiling_document", dataSubfield=None
        )
        ...
        for module in modules:
            ...
            mpayload["time_thread"] = float(module.get("time_thread", 0.0))
            mpayload["time_real"] = float(module.get("time_real", 0.0))
            ...
            mpayload["hash"] = mhash
            notification, _, _ = stomp_amq.make_notification(
                mpayload, "profiling_document", dataSubfield=None
            )
            documents.append(notification)
