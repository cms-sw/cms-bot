...
if $K5COPY ; then
  remote_krbs=$WORKSPACE/../${REMOTE_USER}.cc
  ssh -n $SSH_OPTS ${TARGET} "${SCHEDD_ENV}K5FILE=\$(klist | grep 'FILE:' | sed 's|.*FILE:||') && rsync -v -e 'condor_ssh_to_job' \$K5FILE $GRID_ID:${remote_krbs}"
fi
