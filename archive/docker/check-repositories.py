...
def get_repos(user, cache):
    if user not in cache:
        ...
        while True:
            e, o = subprocess.getstatusoutput("curl -s -L %s" % url)
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--repo-list",
        dest="repo_list",
        help="Yaml file with list of repositories to create under docker hub",
        type=str,
        default=None,
    )
    ...
    if opts.repo_list.startswith("https://"):
        e, o = subprocess.getstatusoutput("curl -s -L %s" % opts.repo_list)
        if e:
            print(o)
            exit(1)
