...
if [ "X$DO_SHORT_MATRIX" = Xtrue -a "X$BUILD_OK" = Xtrue -a "$ONLY_FIREWORKS" = false -a "$RUN_TESTS" = "true" ]; then
  ...
  pushd "$WORKSPACE/runTheMatrix-results"
    ...
    [ $(runTheMatrix.py --help | grep 'command' | wc -l) -gt 0 ] && EXTRA_MATRIX_ARGS="--command '--customise Validation/Performance/TimeMemorySummary.customiseWithTimeMemorySummary' $EXTRA_MATRIX_ARGS"
