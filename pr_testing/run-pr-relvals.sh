...
if [ "X$TEST_ERRORS" != "X" -o "X$GENERAL_ERRORS" = "X" ]; then
  ...
  if [[ -s "$WORKSPACE/bad-workflow-lists.txt" ]]; then
    echo "### Invalid workflow lists" >  ${RESULTS_DIR}/0a-bad-workflows.res
    echo "The following workflow names were not recognized and not ran:" >> ${RESULTS_DIR}/0a-bad-workflows.res
    cat "$WORKSPACE/bad-workflow-lists.txt" >> ${RESULTS_DIR}/0a-bad-workflows.res
  fi
