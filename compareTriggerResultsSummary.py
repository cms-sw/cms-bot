...
def compareTriggerResultsSummary(**kwargs):
    ...
    for inpf in sorted(inputFiles):
        fBasename, wfName = os.path.basename(inpf), os.path.dirname(
            os.path.relpath(inpf, inputDir)
        )
        ...
        try:
            ...
            if verbosity > 10:
                WARNING(
                    "compareTriggerResultsSummary -- failed to extract hltDiff statistics from input file: "
                    + inpf
                )

        if numEventsTotal is None or numEventsWithDiffs is None:
            continue
        ...
        wfDict[wfName][procName] = {
            "numEventsTotal": numEventsTotal,
            "numEventsWithDiffs": numEventsWithDiffs,
        }
    ...
    for wfName in sortedWfNames:
        ...
        for procName in sorted(wfDict[wfName]):
            ...
            if summaryFormat == "html":
                ...
                summaryLines += [
                    "| {:25d} | {:18d} | {:12} | {:}".format(
                        numEventsWithDiffs, numEventsTotal, procName, wfName
                    )
                ]
    ...
    if summaryLines:
        if os.path.exists(summaryFilePath):
            if verbosity > 0:
                WARNING(
                    "compareTriggerResultsSummary -- target output file already exists (summary will not be produced)"
                )
        else:
