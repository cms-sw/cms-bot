...
def pkgCmp(a, b):
    if a.subsys == b.subsys:
        return cmp(a.pkg, b.pkg)
    else:
        return cmp(a.subsys, b.subsys)
...
class LogFileAnalyzer(object):
    ...
    def __init__(
        ...
        if self.topURL != "":
            if self.topURL[-1] != "/":
                self.topURL += "/"
            if not release:
                release = self.topURL.split("/")[-3]  # TODO no error catching
    ...
    def analyze(self):
        ...
        if self.verbose > 0:
            print("going to analyze ", len(packageList), "files.")
    ...
    def report(self):
        ...
        print(
            "found ",
            len(self.pkgErr),
            "packages with errors or warnings, ",
            len(self.pkgWarn),
            " with warnings only.",
        )
        start = time.time()
    ...
    def makeHTMLSummaryPage(self):
        ...
        htmlFile.write(
            "<h3> found " + str(totErr) + " errors and warnings in total, by type: </h3>\n"
        )
        htmlFile.write('<table border="1" cellpadding="10">')
        htmlFile.write(
            "<tr> <td><b>error type</b></td><td><b> # packages </b></td><td><b> total # errors </b></td></tr>\n"
        )
        for key in keyList:
            val = 0
            try:
    ...
    def makeHTMLLogFile(self, pkg):
        ...
        htmlFile.write("<body>\n")
        htmlFile.write(
            "<h2>Log File for " + pkg.name() + "   " + self.tagList[pkg.name()] + "</h2>\n"
        )
        htmlFile.write("<pre>\n")
        lineNo = -1
    ...
    def analyzeFile(self, fileNameIn):
        ...
        if self.verbose > 5:
            print("analyzing file : ", fileNameIn)
        ...
        errorInf = [
            ...
            {
                str("^ *(/.*?/" + self.release + "/|)src/.*?\:\d+\: warning( #[0-9]+-[a-zA-Z]|): "): [
                    "compWarning",
                    "for file in release",
                ]
            },
            ...
            {
                str("^.*: (more undefined references to|undefined reference to).*"): [
                    "compError",
                    "Missing symbols in a package",
                ]
            },
        ]
            ...
            for errI in errors:
                ...
                for errRe, info in errI.items():
                    ...
                    if errMatch:
                        ...
                        if (
                            self.ignoreWarnings
                            and (errTyp == "compWarning")
                            and [w for w in self.ignoreWarnings if w in line]
                        ):
