...
function get_cached_GH_JSON (){
    ...
    if  [ ! -f  ${GH_JSON_PATH} ]; then
        ...
        for i in 0 1 2 3 4 ; do
            ${CMSBOT_PYTHON_CMD} -c "import github_utils,json;github_utils.get_rate_limits();data=github_utils.get_pr('${REPO}', '${PR_NR}');print(json.dumps(data))" > ${GH_JSON_PATH} || continue
            echo ${GH_JSON_PATH}
            break
        done
    else
