...
if ${ALL_OK} ; then
    if [ "${DAS_QUERY_RES}" = "ERROR" ] ; then
      echo "DAS Queries failed" >> ${RESULTS_DIR}/10-report.res
    fi
   ...
   if [ "X$CLANG_BUILD_OK" = Xfalse ];    then TESTS_FAILED="$TESTS_FAILED  ClangBuild" ; fi
    ...
    REPORT_GEN_OPTS="--repo cms-sw/cmssw --pr 1 --report-file ${RESULTS_DIR}/10-report.res ${REPORT_OPTS} "
