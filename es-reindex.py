...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--server", "-s", dest="server", default="localhost:9200", help="the elasticsearch server"
    )
    parser.add_argument(
        "--dry-run",
        "-n",
    ...
    cmd = format(
        "curl -s %(user_string)s '%(server)s/%(source)s/_search?search_type=scan&scroll=1m' -d'%(query)s'",
        source=args.source,
    ...
    while True:
        cmd = format(
            "curl -s %(user_string)s '%(server)s/_search/scroll?scroll=1m' -d'%(scroll_id)s'",
            source=args.source,
            query=json.dumps(query),
            server=args.server,
            user_string=user_string,
        ...
        for item in result["hits"]["hits"]:
            cmd = format(
                '{ "create": { "_index": "%(index)s", "_type": "%(obj_type)s", "_id": "%(obj_id)s" }}',
                index=args.dest,
                obj_type=item["_type"],
                obj_id=item["_id"],
            )
