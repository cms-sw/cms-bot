...
if [ "X$TEST_ERRORS" != "X" -o "X$GENERAL_ERRORS" = "X" ]; then
  ...
  echo "${TEST_FLAVOR_UC}_UNIT_TEST_RESULTS;ERROR,Unit Tests ${TEST_FLAVOR_UC},See Log,${TEST_FLAVOR}UnitTests" >> ${RESULT_FILE_NAME}
  ALL_OK=false
  UNIT_TESTS_OK=false
  $CMS_BOT_DIR/report-pull-request-results PARSE_${TEST_FLAVOR_UC}_UNIT_TESTS_FAIL -f $WORKSPACE/${TEST_FLAVOR}UnitTests/log.txt --report-file ${REPORT_FILE_NAME} ${REPORT_OPTS}
  echo "${TEST_FLAVOR}UnitTests" > ${RESULTS_DIR}/${FAILED_FILE_NAME}
else
  echo "${TEST_FLAVOR_UC}_UNIT_TEST_RESULTS;OK,Unit Tests ${TEST_FLAVOR_UC},See Log,${TEST_FLAVOR}UnitTests" >> ${RESULT_FILE_NAME}
fi
