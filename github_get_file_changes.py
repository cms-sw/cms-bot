...
from _py2with3compatibility import cmp_f, run_cmd
...
def main():
    ...
    parser.add_argument(
        ...
        "--output",
        default=None,
        help="Output result, which is a list of modules that are not"
        " being modified by other PRs.",
    )
    parser.add_argument(
        "-i", "--ignore_modules", default=None, help="Ignore modules which are already done."
    )
    ...
    all_branch_modules_w_mt = get_modules_with_mt(
        args.cloned_repo
    )  # this will return folders 2 levels deep
    ...
    if args.ignore_modules and exists(args.ignore_modules):
        already_done_modules = set(
            ["/".join(d.split("/")[-2:]) for d in glob("%s/*/*" % args.ignore_modules)]
        )
    ...
    print(
        pformat(
            "Modules modified by prs: {} \nAll modules: {} \nModules not touched by prs: {} \nNew modules: {}".format(
                len(modules_mod_by_prs),
                len(all_branch_modules_w_mt),
                len(non_changed_modules),
                len(new_files),
            )
        )
    )
