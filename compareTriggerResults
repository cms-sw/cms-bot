...
def compareTriggerResults(**kwargs):
  ...
  for wfName in sortedWfNames:
    ...
    if not dryRun:
      try:
        ...
        warn_msg = 'target output directory already exists' if os.path.isdir(wfOutputDir) else 'failed to create output directory'
    ...
    for procName in wfDict[wfName]:
      ...
      if summaryFormat == 'html':
        ...
        summaryLines += ['| {:25d} | {:18d} | {:12} | {:}'.format(diffStats[1], diffStats[0], procName, wfName)]
  ...
  if summaryLines:
    outputSummaryFilePath = os.path.join(outputDir, 'index.html' if summaryFormat == 'html' else 'summary.log')
    with open(outputSummaryFilePath, 'w') as outputSummaryFile:
      for _tmp in summaryLines: outputSummaryFile.write(_tmp+'\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file-pattern', dest='file_pattern', action='store', default='*.root',
    ...
    parser.add_argument('-s', '--summary', dest='summary', action='store', default=None, choices=["html", "txt"],
