...
def es_get_passwd(passwd_file=None):
    ...
    for psfile in [
        passwd_file,
        getenv("CMS_ES_SECRET_FILE", None),
        "/data/secrets/cmssdt-es-secret",
        "/build/secrets/cmssdt-es-secret",
        "/var/lib/jenkins/secrets/cmssdt-es-secret",
        "/data/secrets/github_hook_secret_cmsbot",
    ]:
        if not psfile:
            continue
        if exists(psfile):
...
def send_request(
    ...
    header["Authorization"] = (
        "Basic %s" % base64.b64encode(("cmssdt:%s" % passwd).encode()).decode()
    )
    try:
        if payload is None:
...
def send_template(name, payload, passwd_file=None):
    if not name.startswith("cmssdt-"):
        name = "cmssdt-" + name
    uri = "_template/%s" % name
    return send_request(
        uri, payload=payload, passwd_file=passwd_file, method="PUT", ignore_doc=True
    )


def delete_hit(hit, passwd_file=None):
    uri = "%s/%s/%s" % (hit["_index"], "_doc", hit["_id"])
    if not send_request(uri, passwd_file=passwd_file, method="DELETE"):
        return False
...
def get_payload(index, query, scroll=0):
    ...
    if scroll <= 1:
        data["params"] = "ignore_unavailable=true"
...
def get_summary_stats_from_json_file(stats_dict_file_path, cpu_normalize):
    with open(stats_dict_file_path, "r") as stas_d_f:
        stats_dict = json.load(stas_d_f)
    sdata = None
    try:
        xdata = {}
        ...
        for x in xdata:
            ...
            for t in ["min", "max", "avg", "median", "25", "75", "90"]:
                sdata[x + "_" + t] = 0
            if dlen > 0:
                sdata[x + "_min"] = data[0]
                sdata[x + "_max"] = data[-1]
...
def es_send_external_stats(
    ...
    try:
        send_payload(es_index_name + "-" + week, es_doc_name, index_sha, json.dumps(sdata))
    except Exception as e:
        print(str(e))
...
def get_avg_externals_build_stats(
    arch="*", lastNdays=30, cpus=0, memoryGB=0, default_key="90", default_builders=2
):
    if cpus <= 0:
