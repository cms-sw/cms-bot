...
egrps = []
payload = {}
err, cmd_out = subprocess.getstatusoutput(
    "logwatch --range yesterday --detail 10 --service sendmail"
)
if err:
    sys.exit(1)
for line in cmd_out.split("\n"):
    if re.match(match_tmp, line):
        temp_fails.append(line)
    elif "Messages To Recipients:" in line:
        msgs_num = line
    elif "Addressed Recipients:" in line:
        adrpts = line
    elif "Bytes Transferred:" in line:
        byttr = line
    elif re.match(match_hn, line):
        egrps.append(line)
...
temp_fails = dict(
    list(
        map(
            map_int_val,
            list((dict([list(map(cust_strip, x.split(" ")[-3:-1])) for x in temp_fails])).items()),
        )
    )
)
