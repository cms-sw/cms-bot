...
if __name__ == "__main__":
    ...
    if opts.exclude_list:
        ...
        excludedReleases = set([x["cmssw_release"] for x in exclude_list["results"]]).union(
            WHITELIST
        )
        excludedBaseReleases = set(
            [re.sub("_[a-zA-Z0-9]*patch[0-9]*", "", x) for x in excludedReleases]
        )
