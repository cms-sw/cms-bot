...
java -jar /var/lib/jenkins/jenkins-cli.jar -s http://localhost:8080/jenkins groovy /var/lib/jenkins/lxplus-labels.groovy $WORKER_JENKINS_NAME "$REAL_ARCH" $DELETE_SLAVE `echo $TARGET | sed 's|.*@||'` $CMS_ARCH
