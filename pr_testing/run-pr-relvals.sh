...
if [ "X$TEST_ERRORS" != "X" -o "X$GENERAL_ERRORS" = "X" ]; then
  ...
  echo "MATRIX${UC_TEST_FLAVOR}_TESTS;ERROR,Matrix ${UC_TEST_FLAVOR} Tests Outputs,See Logs,runTheMatrix${UC_TEST_FLAVOR}-results" >> $(get_status_file_name relval $TEST_FLAVOR)
  ALL_OK=false
  RELVALS_OK=false
  $CMS_BOT_DIR/report-pull-request-results PARSE_MATRIX_FAIL -f ${LOG} --report-file ${RESULT_FILE_NAME} --report-url ${PR_RESULT_URL} $NO_POST
  if [ $(grep -v '## RelVals' ${RESULT_FILE_NAME} | grep -v '^ *$' | wc -l) -eq 0 ] ; then
    echo -e "## RelVals\n\n\`\`\`\n${TEST_ERRORS}\n\`\`\`\n" > ${RESULT_FILE_NAME}
  fi
  if [ "${TEST_FLAVOR}" != "" ] ; then
    sed -i -e "s|## RelVals|## RelVals-${UC_TEST_FLAVOR}|;s|/runTheMatrix-results|/runTheMatrix${UC_TEST_FLAVOR}-results|g" ${RESULT_FILE_NAME}
    echo "RelVals-${UC_TEST_FLAVOR}" > ${FAILED_FILE_NAME}
  else
    echo "RelVals" > ${FAILED_FILE_NAME}
  fi
  mark_commit_status_all_prs "${GH_COMP_CONTEXT}" 'success' -d "Not run due to failure in relvals" ${MARK_OPTS}
else
  touch ${FAILED_FILE_NAME}
  touch ${RESULT_FILE_NAME}
  echo "no errors in the RelVals!!"
  echo "MATRIX${UC_TEST_FLAVOR}_TESTS;OK,Matrix ${UC_TEST_FLAVOR} Tests Outputs,See Logs,runTheMatrix${UC_TEST_FLAVOR}-results" >> $(get_status_file_name relval $TEST_FLAVOR)
