...
class JobReportHandler(ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        path = ".".join(
            ["IBRelVals", self.architecture, self.release, self.workflow, self.step, attrs["Name"]]
        )
        value = attrs["Value"]
...
def calculateFileSizeMetrics(release, architecture, timestamp, fullRelease, args):
    ...
    error, out = subprocess.getstatusoutput(format(cmd))
    if error:
        return
    if not out.strip():
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base-dir", dest="baseDir", default=IB_BASE_DIR, help="Where the logs are located."
    )
    parser.add_argument(
        "--server", dest="server", default=CARBON_SERVER, help="Where the logs are located."
    )
    parser.add_argument(
        "--port", dest="port", default=CARBON_PORT, help="Where the logs are located."
    )
    parser.add_argument(
        "--filter-release", dest="filterRelease", default=".*", help="regexp to filter releases"
    )
    parser.add_argument(
        "--filter-workflows",
        dest="filterWorkflows",
        default=".*",
        help="regexp to filter releases",
    )
    parser.add_argument(
        "--days", dest="days", default=7, type=int, help="days to go into the past."
    )
    parser.add_argument(
        "what", metavar="KEYS", type=str, nargs="*", help="What to dump from the logs"
    )
    ...
    print(cmd)
    error, files = subprocess.getstatusoutput(cmd)
    ...
    for f in files:
        ...
        date = re.sub(
            ".*/CMSSW_[^/]*(20[0-9][0-9]-[0-1][0-9]-[0-3][0-9]-[0-2][0-9][0-9][0-9]).*", "\\1", f
        )
        ...
        if not re.match(args.filterRelease, release):
            continue
        error, reports = subprocess.getstatusoutput(
            "unzip -l %s | grep JobReport | awk '{print $4}'" % f
        )
        ...
        for r in [x for x in reports.split("\n") if x]:
            cmd = "unzip -p %s %s" % (f, r)
            error, report = subprocess.getstatusoutput(cmd)
            ...
            if not args.what:
                ...
                handler = JobReportHandler(
                    args.what, step, architecture, release, workflow, timestamp
                )
            try:
