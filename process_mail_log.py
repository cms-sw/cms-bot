...
def map_int_val(pair):
  ...
  return key , int(val)
#get relevant info
err,timestpline = getstatusoutput("grep 'Processing Initiated:' " + logFile)
...
#process info
timestamp = time.mktime(datetime.datetime.strptime(timestpline[30:],'%a %b %d %H:%M:%S %Y').timetuple())
