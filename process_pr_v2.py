...
def deterministic_hash(s: str) -> int:
    ...
    Unlike Python's built-in hash(), this returns the same value across
    different Python processes (hash randomization doesn't affect it).
...
def handle_ignore_tests_rejected(context: PRContext, match: re.Match, comment: Any) -> bool:
    ...
    # Check if tests are actually rejected (check raw status before setting ignore flag)
    # We need to check the underlying CI status, not the approval state
    lab_stats = check_ci_test_completion(context)
    if not lab_stats:
        logger.info("No test results yet, ignore command has no effect")
        return True  # Command is valid but has no effect

    required_status = lab_stats.get("required")
    if required_status != "error":
        logger.info(
            f"Tests are not failing (status: {required_status}), ignore command has no effect"
        )
        return True  # Command is valid but has no effect

    context.ignore_tests_rejected = reason

    # Add tests-{reason} label to indicate override
    context.pending_labels.add(f"tests-{reason}")
...
def handle_test_parameters(context: PRContext, match: re.Match, comment: Any) -> bool:
    ...
    context.test_params_comment_id = comment.id
...
def handle_build_test(context: PRContext, match: re.Match, comment: Any) -> bool:
    ...
    if result.verb == "build" and get_bot_reaction(context, comment_id):
    ...
    if context.pending_build_test_command:
        old_comment, _ = context.pending_build_test_command
        logger.info(
            f"Replacing old build/test command from comment {old_comment.id} by {old_comment.user.login}"
        )
...
def get_bot_reaction(context: PRContext, comment_id: int) -> Optional[str]:
    """
    Get the bot's reaction on a comment.
    ...
    Returns:
        Reaction type (e.g., '+1' or '-1') from bot, or None if no reaction
    """
...
def process_pending_build_test_commands(context: PRContext) -> None:
    ...
    comment, result = context.pending_build_test_command
    comment_id = comment.id
    user = comment.user.login
    timestamp = get_comment_timestamp(comment)
    comment_url = getattr(comment, "html_url", None) or get_comment_url(context, comment_id)
...
def update_file_states(context: PRContext) -> Tuple[Set[str], Set[str]]:
    ...
    # Get commit timestamp (PyGithub returns datetime)
    now = datetime.now(tz=timezone.utc).isoformat().replace("+00:00", "Z")
    if context.last_commit:
        try:
            commit_ts = ensure_tz_aware(context.last_commit.commit.author.date)
...
def get_comment_timestamp(comment) -> datetime:
    ...
    PyGithub returns datetime objects for timestamps. We just ensure they're tz-aware.
    """
    return ensure_tz_aware(comment.created_at)
...
def process_comment(context: PRContext, comment, use_cached: bool = False) -> None:
    ...
    for cmd, match in context.command_registry.find_commands(command_line, is_pr=context.is_pr):
        ...
        if should_skip_command_before_latest_commit(context, cmd, command_line, timestamp, is_bot):
            # Only cache signature commands (approve/reject) - other commands don't need caching
            if not use_cached and cmd.name in ("approve", "reject"):
...
def get_last_commit_timestamp(context: PRContext) -> Optional[datetime]:
    """
    Get timestamp of the last commit in the PR.

    Returns:
        Timestamp of last commit, or None if no commits
    """
    if not context.last_commit:
        return None
...
def has_commit_after(last_commit_ts: Optional[datetime], comment_timestamp: datetime) -> bool:
    """
    Check if the last commit is after the given comment timestamp.

    Args:
        last_commit_ts: Timestamp of the last commit, or None
        comment_timestamp: Timestamp to check against

    Returns:
        True if the last commit is after the comment timestamp
    """
    if last_commit_ts is None:
        return False
    return last_commit_ts > comment_timestamp
...
def process_all_comments(context: PRContext) -> None:
    ...
    for comment_id, comment_info in context.cache.comments.items():
        ...
        if comment_ts and has_commit_after(last_commit_ts, comment_ts):
    ...
    for suffix in ["required", "optional"]:
        ...
        for result in results:
            ...
            if output:
                ...
                # the same test run.
                url_hash = deterministic_hash(result.target_url) if result.target_url else head_sha
...
def generate_status_message(context: PRContext) -> str:
    ...
    if context.is_pr:
        ...
        if pr_state == PRState.MERGED:
            lines.append("✅ **PR has been merged**")
        elif can_merge(context):
            ...
            if reasons:
                ...
                # No specific reasons found but can_merge is False
...
def build_test_parameters(context: PRContext, test_request: "TestRequest") -> Dict[str, str]:
    ...
    if test_request.verb == "build":
        ...
        params["CONTEXT_PREFIX"] = f"cms/{pr_id}/build"
...
def post_welcome_message(context: PRContext) -> None:
    ...
    # For PRs: mention L2s of affected categories
    # For Issues: mention CMSSW_ISSUES_TRACKERS
    gh_user_char = "@" if not context.notify_without_at else ""
    if context.is_pr:
        all_l2s = set()
        for cat in context.signing_categories:
            cat_l2s = get_category_l2s(context.repo_config, cat, timestamp)
        ...
        l2_mentions = ", ".join(gh_user_char + name for name in sorted(CMSSW_ISSUES_TRACKERS))
...
def check_for_new_commits(context: PRContext) -> None:
    ...
    if has_bot_comment(context, "pr_updated", deterministic_hash(latest_sha)):
