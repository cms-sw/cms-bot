...
#                    print "UNKNOWN error found for ", subsys, pkg, ":\n     ", line
        ...
        if self.htmlOut :
            ...
            for key, val in subSysCompErr.items():
                htmlFile.write( '<p>\n')
                htmlFile.write( '<a href="'+htmlFileName+'#'+key+'">'+str(len(val))+' Compile ERRORs found for subsystem ' + key )
                htmlFile.write( '</a> <br />\n')
                htmlFile.write( '</p>\n')

            for key, val in subSysLinkErr.items():
                htmlFile.write( '<p>\n')
                htmlFile.write( '<a href="'+htmlFileName+'#'+key+'">'+str(len(val))+' Linker ERRORs found for subsystem ' + key )
                htmlFile.write( '</a> <br />\n')
                htmlFile.write( '</p>\n')
            ...
            for key, val in subSysCompErr.items():
                ...
                for item in val:
                    pkg = item[0]
                    try:
                        htmlFile.write( '<li>package '+pkg+' file '+item[1]+' Tag: '+self.pkgVers[key+"/"+pkg]+' <br />\n')
                    except KeyError:
            ...
            for key, val in subSysGenErr.items():
                htmlFile.write( '<p>\n')
                htmlFile.write( '<a href="'+htmlFileName+'#'+key+'">'+str(len(val))+' UNKNOWN ERRORs found ' )
                htmlFile.write( '</a> <br />\n')
                htmlFile.write( '</p>\n')
	...
	if len(errorList.items()) > 500 :
            if (self.htmlOut and self.sumLog ) :
                self.sumLog.write( '<font color="#ff0000"><b>Caution:</b> Too many errors found ("+len(errorList.items())+"), printout suppressed !!</font>\n')
                errLimit = True
