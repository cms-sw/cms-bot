...
def create_comp_package(arch, pkg, ver, cmspkg):
  ...
  if pkg=="python3":
    ...
    if not exists(comp_init):
      ...
      cmd = "cp -fp {0} {1} && sed -i -e '/dependencies-setup/d;/PYTHON27PATH=/d;/LD_LIBRARY_PATH=/d' {1}".format(cms_init, comp_init)
    ...
    if not exists(def_future):
      ...
      runcmd("mkdir -p {0} && ln -s {1} {2}".format(dirname(def_future), comp_ver, def_future))
  else:
