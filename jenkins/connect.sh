...
case $WORKER_NODE in
  ...
  * ) java -jar ${JENKINS_MASTER_ROOT}/jenkins-cli-2.46.2.jar -i /home/jenkins/.ssh/id_dsa -s http://localhost:8080/jenkins -remoting groovy ${SCRIPT_DIR}/add-cpu-labels.groovy "${WORKER_NAME}" "${HOST_ARCH}" "${HOST_CMS_ARCH}" "${DOCKER}" ;;
esac
